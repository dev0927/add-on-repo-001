import urllib,urllib2, re,base64, time, math, json
...
def item_resolver(encoded_url):
    ...
    for item in items:
        data = {}
        data['name']=item['name']#.encode('base64', 'strict').replace('\n', '')
        data['url']=item['url']#.encode('base64', 'strict').replace('\n', '')
        list.append(data)

    return list

print(item_resolver('aHR0cDovL3R2bGlzdC5iaXovdHZ0di9nZXRfbGlzdC5waHA/az1ramFncnV0Z2prdm5ia2FsaA=='))